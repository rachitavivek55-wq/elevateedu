...
(function () {
  ...
  add('meta', { name: 'theme-color', content: '#473a36' }, 'meta[name=\'theme-color\']');
...
var elevateAuth = (function () {
  ...
  function installCard(before) {
    ...
    var html = '<div style="font-size:12.5px;font-weight:700;color:#473a36;margin-bottom:7px;">'
      ...
      + '<ol style="margin:0;padding-left:17px;font-size:11.5px;line-height:1.65;color:#67503f;">';
    for (var i = 0; i < steps.length; i++) html += '<li>' + steps[i] + '</li>';
    html += '</ol><div style="font-size:10.5px;line-height:1.5;color:#67503f;opacity:.75;'
  ...
  function codeBox() {
    ...
    inp.style.cssText = 'flex:1;min-width:0;padding:13px;border-radius:14px;border:1px solid rgba(71,58,54,0.28);background:#fefcf9;color:#473a36;font-size:15px;font-family:inherit;';
    var btn = document.createElement('button');
    btn.type = 'button';
    btn.textContent = 'Sign in';
    btn.style.cssText = 'border:none;background:#67503f;color:#efe3d0;border-radius:14px;padding:13px 18px;font-weight:700;font-size:15px;font-family:inherit;cursor:pointer;';
    ...
    if (navigator.clipboard && navigator.clipboard.readText) {
      ...
      pb.style.cssText = 'margin-top:9px;width:100%;border:1px solid rgba(71,58,54,0.28);background:transparent;color:#473a36;border-radius:14px;padding:12px;font-weight:700;font-size:14px;cursor:pointer;';
...
(function () {
  ...
  function eeRoomNote() {
    ...
    try {
      ...
      n.style.background = signedIn ? "#372f29" : "#7c2f22";
      n.style.color = "#fff";
...
(function(){
  ...
  function makeBtn(){
    ...
    b.style.cssText = 'position:fixed;left:50%;transform:translateX(-50%);bottom:86px;z-index:9999;'+
      ...
      'box-shadow:0 6px 18px rgba(71,58,54,0.28);cursor:pointer;opacity:0;transition:opacity .3s;';
...
/* ============================================================
   ...
   ============================================================ */
(function(){
  var COFFEE = '#67503f', CREAM = '#efe3d0', ESPRESSO = '#473a36';
  function installed(){
    return (window.matchMedia && window.matchMedia('(display-mode: standalone)').matches) || window.navigator.standalone === true;
  }
  ...
  function stepsHtml(){
    ...
    function tip(t){ return '<div style="font-size:13px;margin-top:12px;padding-top:10px;border-top:1px solid rgba(71,58,54,0.18);opacity:0.85;">' + t + '</div>'; }
  ...
  function showCard(){
    ...
    if (isIOS || iPadHere) {
      ...
      if (hs) {
        ...
        hw.style.cssText = 'margin-top:16px;padding:12px;border-radius:12px;background:rgba(103,80,63,0.10);';
...
(function () {
  ...
  function checkAndPrompt() {
    ...
    hint.style.cssText = "margin-top:14px;font-size:13px;line-height:1.5;color:#67503f;text-align:center;";
...
(function () {
  var CSS =
    '#eeGear{width:34px;height:34px;border-radius:12px;border:0;padding:0;background:var(--tile,#f8f2e9);box-shadow:var(--shadow-soft,0 4px 14px rgba(71,58,54,.06));color:#67503f;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;transition:transform .15s ease}' +
    '#eeGear:active{transform:scale(.9)}' +
    '#eeGear svg{width:18px;height:18px}' +
    '#eeFeed{width:34px;height:34px;border-radius:12px;border:0;padding:0;background:var(--tile,#f8f2e9);box-shadow:var(--shadow-soft,0 4px 14px rgba(71,58,54,.06));color:#67503f;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;transition:transform .15s ease}' +
    ...
    '#eeFbCard{background:var(--card,#fdf9f3);border-radius:20px;max-width:360px;width:100%;box-sizing:border-box;padding:22px 20px;box-shadow:0 18px 50px rgba(71,58,54,.25);text-align:center}' +
    '#eeFbIcon{width:46px;height:46px;border-radius:15px;background:#efe3d0;color:#67503f;display:flex;align-items:center;justify-content:center;margin:0 auto 12px}' +
    '#eeFbIcon svg{width:23px;height:23px}' +
    '#eeFbCard h3{margin:0 0 8px;font-size:17px;color:#473a36}' +
    '#eeFbCard p{margin:0 0 16px;font-size:14px;line-height:1.55;color:#6a5c50}' +
    '#eeFbGo{display:block;width:100%;box-sizing:border-box;text-decoration:none;background:#67503f;color:#fff;border:0;border-radius:14px;padding:13px 16px;font-size:15px!important;font-weight:600;font-family:inherit;cursor:pointer}' +
    '#eeFbLater{display:block;width:100%;margin-top:8px;background:transparent;border:0;color:#82725d;font-size:14px!important;font-family:inherit;padding:10px;cursor:pointer}' +
    ...
    '#eeSetOverlay{position:fixed;inset:0;z-index:99998;background:rgba(71,58,54,.42);display:none;align-items:flex-end;justify-content:center}' +
    '#eeSetOverlay.ee-open{display:flex}' +
    '#eeSetCard{width:100%;max-width:430px;background:#f8f2e9;border-radius:26px 26px 0 0;padding:18px 20px 26px;max-height:88vh;overflow:auto;box-shadow:0 -12px 34px rgba(71,58,54,.20);animation:eeSetUp .22s ease}' +
    '@keyframes eeSetUp{from{transform:translateY(28px);opacity:.5}to{transform:translateY(0);opacity:1}}' +
    '#eeSetCard h3{margin:0;font-size:17px;color:#473a36;font-weight:600}' +
    '.eeSetTop{display:flex;align-items:center;justify-content:space-between;margin-bottom:4px}' +
    '#eeSetClose{border:0;background:#eae0d1;color:#67503f;width:30px;height:30px;border-radius:50%;font-size:17px;line-height:1;cursor:pointer;font-family:inherit}' +
    '#eeSetWho{font-size:12px;color:#837063;margin:0 0 6px;word-break:break-all}' +
    '.eeSetRow{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 0;border-bottom:1px solid rgba(103,80,63,.10)}' +
    '.eeSetRow .eeL{flex:1 1 auto}' +
    '.eeSetRow .eeL b{display:block;font-size:14px;color:#473a36;font-weight:600}' +
    '.eeSetRow .eeL i{display:block;font-style:normal;font-size:11.5px;color:#837063;margin-top:3px;line-height:1.35}' +
    '.eeSetBtn{border:0;border-radius:14px;padding:9px 13px;font-size:12.5px;font-weight:600;cursor:pointer;background:#eae0d1;color:#473a36;white-space:nowrap;font-family:inherit}' +
    '.eeSetBtn.ee-warn{background:#eadccc;color:#7c5b3d}' +
    '.eeSetBtn.ee-danger{background:#ddbdae;color:#5f3524}' +
    '.eeSetBtn:active{transform:scale(.96)}' +
    '.eeSetBtn[disabled]{opacity:.55}' +
    '#eeSetConfirm{display:none;margin-top:12px;background:#f4ece3;border:1px solid rgba(103,80,63,.16);border-radius:16px;padding:12px}' +
    '#eeSetConfirmText{margin:0 0 8px;font-size:12.5px;color:#67503f;line-height:1.45}' +
    '#eeSetType{width:100%;box-sizing:border-box;border:1px solid rgba(103,80,63,.25);border-radius:12px;padding:9px 10px;font-size:13px;font-family:inherit;color:#473a36;background:#fff;margin-bottom:8px}' +
    '#eeSetFoot{margin-top:14px;text-align:center;font-size:11.5px;color:#837063;line-height:1.6}' +
    '#eeSetFoot a{color:#67503f;text-decoration:underline;margin:0 6px}';
  ...
  function askFor(kind) {
    ...
    inp.style.borderColor = 'rgba(103,80,63,.25)';
